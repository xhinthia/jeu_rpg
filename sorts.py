def degat_pv(force,vieup,lootpv,defensem,degpv,keltu,ritsang):
	...
	if int(keltu)==2:
		...
		if int(degpv)<500:
			...
			degat=(force+96+32*vieup+lootpv)//3-defensem
...
def degat_vitesse(force,vitesse,stackvit,defensem,degvit,azzin,oth):
	...
	if int(azzin)==2:
		if int(oth)<500:
			...
			degat=(force+vitesse)*5//2+stackvit*stackvit-defensem
		elif int(oth)>=5000:
			degat=(force+vitesse)*3+stackvit*stackvit-defensem
		if int(degat)<0:
			degat=0
	else:
		if int(degvit)<500:
